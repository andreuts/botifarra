...
export function GamePage() {
  ...
  const { gameState, connected, error, gameResult, toasts, dismissToast, reset, setActiveGameRoomId, activeGameRoomId } = useGameStore();
      ...
      <div className="page">
        <div style={{ textAlign: 'center' }}>
          <div style={{
            ...
            animation: 'pulse 0.8s linear infinite', margin: '0 auto 1rem',
          }} />
  ...
  const isContraPhase = gameState.trump !== null
  ...
  return (
    <div style={{
      maxWidth: 700, margin: '0 auto', padding: '0.75rem',
      display: 'flex', flexDirection: 'column', gap: '0.75rem',
      minHeight: '100dvh', position: 'relative',
    }}>
      {/* Toast */}
      {toasts.length > 0 && (
        <div className="toast" onClick={dismissToast}>{toasts[0]}</div>
      )}
      ...
      {gameResult && (
        <div style={{
          ...
          background: gameResult.winner === myTeam
          ...
          zIndex: 200, animation: 'fadeIn 0.5s ease',
        }}>
          <div style={{
            textAlign: 'center',
            maxWidth: 600,
              ...
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '3rem' }}>
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{
                    ...
                    textShadow: gameResult.winner === 0 ? '0 0 20px rgba(46, 204, 113, 0.8)' : 'none',
                  }}>
                    {gameResult.scores[0]}
                  </div>
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{
                    ...
                    textShadow: gameResult.winner === 1 ? '0 0 20px rgba(46, 204, 113, 0.8)' : 'none',
                  }}>
                    {gameResult.scores[1]}
                  </div>
                  <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.5)', marginTop: '0.25rem' }}>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.75rem' }}>
          {mode === 'practice' && (
            <span style={{ background: 'var(--color-surface-2)', padding: '0.2rem 0.5rem', borderRadius: 'var(--radius)', color: 'var(--color-warning)' }}>Practice</span>
          )}
      ...
      {/* Dealer indicator */}
      <div style={{
        textAlign: 'center', fontSize: '0.75rem', color: 'var(--color-muted)',
        padding: '0.25rem',
      }}>
      ...
      {(isDeclaringPhase || isContraPhase) && (
        <DeclareTrumpPanel gameState={gameState} mySeat={mySeat} onDeclare={sendDeclareTrump} onPass={sendPassDeclaration} onContra={sendCallContra} />
      )}
      ...
      <div style={{
        ...
        textAlign: 'center', fontWeight: isMyTurn ? 600 : 400,
        transition: 'all 0.2s',
      }}>
        {isMyTurn
