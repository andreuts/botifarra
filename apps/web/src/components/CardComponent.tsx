...
const RANK_NAME: Record<number, string> = {
  ...
  8: '8', 9: '9', 10: 'Sota', 11: 'Cavall', 12: 'Rei',
};
...
function SuitPip({ suit, size }: { suit: Card['suit']; size: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none" strokeLinecap="round" style={{ display: 'block' }}>
...
const PIP_LAYOUT: Record<number, [number, number][]> = {
  ...
  3: [[0.50, 0.15], [0.50, 0.50], [0.50, 0.85]],
  4: [[0.22, 0.18], [0.78, 0.18], [0.22, 0.82], [0.78, 0.82]],
  5: [[0.22, 0.18], [0.78, 0.18], [0.50, 0.50], [0.22, 0.82], [0.78, 0.82]],
  6: [[0.22, 0.15], [0.78, 0.15], [0.22, 0.50], [0.78, 0.50], [0.22, 0.85], [0.78, 0.85]],
  7: [
    [0.50, 0.08],
    [0.22, 0.25], [0.78, 0.25],
    [0.22, 0.50], [0.78, 0.50],
...
const ZONE_Y: [number, number] = [0.24, 0.80];
...
function CardFace({ card, w, h, small }: { card: Card; w: number; h: number; small: boolean }) {
  ...
  if (COURT_RANKS.has(card.rank)) {
    ...
    return (
      <>
        <div style={{
          position: 'absolute', inset: small ? 5 : 7,
        ...
        <div style={{
          ...
          {!small && (
            <span style={{
              ...
              opacity: 0.65, fontFamily: 'Georgia, serif',
            }}>
              {RANK_NAME[card.rank]}
            </span>
          )}
  ...
  return (
    <>
      {pips.map(([xf, yf], i) => {
        ...
        return (
          <div key={i} style={{ position: 'absolute', left, top, width: pipSize, height: pipSize, lineHeight: 0 }}>
...
export function CardComponent({
  ...
  dealDelay = 0, animate = false, playAnimate = false, rotation = 0,
}: CardProps) {
  const w = small ? 50 : 74;
  ...
  return (
    <div
      ...
      onFocus={(e) => {
        if (interactable) (e.currentTarget as HTMLElement).style.boxShadow =
      ...
      {faceDown ? (
        ...
        <>
          ...
          <div style={{
            ...
            <span style={{
              fontSize: cornerFs, fontWeight: 800, color,
          ...
          <div style={{
            ...
            display: 'flex', flexDirection: 'column', alignItems: 'center',
            lineHeight: 1, transform: 'rotate(180deg)', zIndex: 1,
          }}>
            <span style={{
              fontSize: cornerFs, fontWeight: 800, color,
              fontFamily: 'Georgia, serif', lineHeight: 1,
  ...
  return (
    <div
      ...
      style={{
        ...
        color: 'rgba(201,168,76,0.2)', fontSize: '0.7rem',
